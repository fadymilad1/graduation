...
export default function BusinessInfoPage() {
  ...
  // Load saved business info and isPublished when page mounts
  useEffect(() => {
    try {
      let isHospital = true
      try {
        ...
        if (u) {
          ...
          isHospital = (parsed?.businessType || parsed?.business_type || 'hospital') === 'hospital'
        }
      } catch {
        isHospital = true
      }
      const saved = getScopedItem('businessInfo') ?? (isHospital ? localStorage.getItem('businessInfo') : null)
      if (saved) {
      ...
      // Load isPublished so we show the right UI (View Website vs Publish button)
      const published = getScopedItem('isPublished') === 'true' || (isHospital && localStorage.getItem('isPublished') === 'true')
  ...
  // Auto-save business info draft whenever the user types (scoped when logged in, else global)
  useEffect(() => {
    try {
      const { logo, ...rest } = formData
      // Preserve logo: use logoPreview (set on upload or load), else existing from storage
      const existing = getScopedItem('businessInfo') ?? localStorage.getItem('businessInfo')
      const existingLogo = existing ? (() => { try { return JSON.parse(existing).logo } catch { return null } })() : null
      const logoToSave = logoPreview ?? existingLogo ?? null
      const toSave = { ...rest, logo: logoToSave }
      setScopedItem('businessInfo', JSON.stringify(toSave))
      // For hospital: also write to global so it persists when navigating
      const u = localStorage.getItem('user')
      const isHospital = u ? (() => { try { const p = JSON.parse(u); return (p?.businessType || p?.business_type || 'hospital') === 'hospital' } catch { return true } })() : true
      if (isHospital) localStorage.setItem('businessInfo', JSON.stringify(toSave))
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Convert logo File to base64 data URL if present; otherwise use logoPreview (from storage)
      ...
      // For hospital: also copy to global storage so the site persists when navigating back/forth
      if (userType === 'hospital') {
        localStorage.setItem('businessInfo', JSON.stringify(businessInfoToSave))
        localStorage.setItem('isPublished', 'true')
        const selectedFeatures = getScopedItem('selectedFeatures') ?? localStorage.getItem('selectedFeatures')
        if (selectedFeatures) localStorage.setItem('selectedFeatures', selectedFeatures)
      }
      
      setIsPublishing(false)
      setIsPublished(true)
      
      // Redirect to live website (short delay so user sees success message)
      setTimeout(() => {
        if (userType === 'pharmacy') {
          ...
          router.push('/templates/hospital')
        }
      }, 800)
    } catch (err) {
                ...
                onChange={(file) => {
                  ...
                  if (file) {
                    ...
                    reader.onloadend = () => {
                      ...
                      try {
                        const saved = getScopedItem('businessInfo') ?? localStorage.getItem('businessInfo')
                        const parsed = saved ? JSON.parse(saved) : {}
                        parsed.logo = dataUrl
                        const str = JSON.stringify(parsed)
                        setScopedItem('businessInfo', str)
                        const u = localStorage.getItem('user')
                        const isHospital = u ? (() => { try { const p = JSON.parse(u); return (p?.businessType || p?.business_type || 'hospital') === 'hospital' } catch { return true } })() : true
                        if (isHospital) localStorage.setItem('businessInfo', str)
                      } catch {
                        // Ignore
                      }
                    }
          ...
          {isPublished ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-neutral-gray mb-6">
                Your website is now live. You can view it below or from the dashboard.
              </p>
              <Button
                variant="primary"
                onClick={() => router.push(userType === 'pharmacy' ? `/templates/pharmacy/${getScopedItem('selectedTemplate') ? parseInt(getScopedItem('selectedTemplate')!) : 1}` : '/templates/hospital')}
              >
